...
const ListItemMenu = (props: ListItemMenuProps): React.ReactElement | null => {
	...
	return (
		<ListItemMenuView {...props} selected={selected} />
	)
}
...
const ListItemMenuView = ({arr = [], selected}: ListItemMenuViewProps): React.ReactElement | null => {
	if (!(arr) || arr.length === 0) {
		return null
	}
	const itemSelected = arr[selected];

	const renderTextSelection = (selected !== -1 && itemSelected) ? `${itemSelected.id}${chalk.inverse(' ')}` : '';
			...
			<Box {...styles.wrapperContent}>
				{
					arr.map((item, index) => {
						if (index === 0) {
							return <ItemMenu {...item} key={item.id}/>
						}
						return <ItemMenu {...item} key={item.id} style={{marginTop: 1}}/>
					})
				}
			</Box>
			<Box {...styles.wrapperControl}>
				<Text>Enter your selection [{arr.map(item => item.id).toString()}] and
					press <Text {...styles.wrapperControl_TextEnter}>Enter</Text>: {renderTextSelection}</Text>
				<Spacer />
				<Text>Press <Text {...styles.wrapperContent_TextQuit}>q</Text> to Quit.</Text>
			</Box>
		</Box>
	)
};
