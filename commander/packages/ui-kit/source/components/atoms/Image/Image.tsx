...
const Image = (props: ImageProps): React.ReactElement | null => {
  ...
  if (imageStr) {
    return <ImageLayout imageStr={imageStr} {...props} />;
  }
  return null;
};
...
const ImageLayout = ({
  ...
  const renderBoxHorizontal = (x?: number): React.ReactElement | null => {
    if (
      typeof x === 'number' &&
      x > 0 &&
      detectTerminalMacOS() === TerminalMacOs.DEFAULT
    ) {
      return <Box width={x} />;
    }
    return null;
  ...
  const renderBoxHorizontalOnIterm = (x?: number): string => {
    if (
      typeof x === 'number' &&
      x > 0 &&
      detectTerminalMacOS() === TerminalMacOs.ITERM
    ) {
  ...
  return (
    <Box {...displayColumn}>
      ...
      <Box {...displayRow}>
        ...
        <Text>
          {renderBoxHorizontalOnIterm(marginLeft)}
          {imageStr}
          {renderBoxHorizontalOnIterm(marginRight)}
        </Text>
...
export type { ImageProps, ImageLayoutProps, Spacing };
